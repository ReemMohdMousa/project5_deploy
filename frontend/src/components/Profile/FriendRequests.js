...
const FriendRequests = ({ id }) => {
  ...
  const addFriendFun = (id) => {
    axios
      .post(
        `https://project5-deploy.onrender.com/friends/add`,
        { user2_id: id },
        {
          headers: { Authorization: `Bearer ${token}` },
        }
  ...
  const acceptFriendReq = () => {
    axios
      ...
      .then(function (response) {
        ...
        // axios
        //   .get(`https://project5-deploy.onrender.com/users/others/info/${friendId}`)
        //   .then((response) => {
        //     console.log(response.data.result);
  ...
  const cancelFriendReqFun = () => {
    axios
      .delete(`https://project5-deploy.onrender.com/friends/cancel/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then(function (response) {
        response.data.result.map((element, i) => {
  ...
  const declineFriendReqFun = () => {
    axios
      .delete(`https://project5-deploy.onrender.com/friends/decline/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then(function (response) {
        // console.log(response.data.result);
